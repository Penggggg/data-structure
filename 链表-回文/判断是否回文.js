 ...
 *
 * Tips1:
 * 慢指针为参考系，快指针每次相对慢指针向前走一步，如果快指针到达了尾巴，则慢指针刚好在链表中间
 *
 * Tips2:
 * 递归，如果每次执行都给上一次返回一些值，那每次执行之间都可以进行数据通讯
 * 如果只有最后一次有返回，那总执行（这个递归本身）只会返回，最后一次执行都那个值
 *
 */
var isPalindrome = function (start) {
    ...
    /** 拿到下半段，反转后的第一个值 */
    var newStart = start;
    var newHalfStart = reverse(undefined, halfStart);
    if (!newHalfStart) {
        return false;
    }
    /** 下一半 跟 上一半 进行对比 */
