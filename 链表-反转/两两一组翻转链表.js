 ...
 *
 */
var reverseTwins1 = function (item) {
    if (!item) {
        return;
    }
    // 第一个框（框住2元素）的开头元素
    var firstKuangStart = item;
    while (firstKuangStart) {
        if (!firstKuangStart) {
            return;
        }
        // 第一个框的结尾元素
        var firstKuangEnd = firstKuangStart.next;
        if (!firstKuangEnd) {
            return;
        }
        // 下一个框的开头元素
        var secondKuangStart = firstKuangEnd.next;
        if (!secondKuangStart) {
            return;
        }
        // 下一个框的结尾元素
        var secondKuangEnd = secondKuangStart.next;
        // 第一个框内部翻转
 ...
 * 方法二，同上，递归
 */
var reverseTwins2 = function (item) {
    if (!item) {
        return;
    }
    var innerLoop = function (firstKuangStart) {
        if (!firstKuangStart) {
            return;
        }
        // 第一个框的结尾元素
 ...
 */
var reverseTwins3 = function (kuangStart) {
    if (!kuangStart) {
        return;
    }
    var kuangEnd = kuangStart.next;
    var KuangNext = !!kuangEnd ? kuangEnd.next : undefined;
